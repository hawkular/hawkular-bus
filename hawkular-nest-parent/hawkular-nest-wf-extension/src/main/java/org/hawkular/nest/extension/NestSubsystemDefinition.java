...
public class NestSubsystemDefinition extends SimpleResourceDefinition {
    ...
    protected static final SimpleAttributeDefinition AGENT_ENABLED_ATTRIBDEF = new SimpleAttributeDefinitionBuilder(NestSubsystemExtension.NEST_ENABLED_ATTR,
            ModelType.BOOLEAN).setAllowExpression(true).setXmlName(NestSubsystemExtension.NEST_ENABLED_ATTR)
            .setFlags(AttributeAccess.Flag.RESTART_RESOURCE_SERVICES).setDefaultValue(new ModelNode(NestSubsystemExtension.NEST_ENABLED_DEFAULT))
            .setAllowNull(false).build();

    protected static final SimpleAttributeDefinition AGENT_NAME_ATTRIBDEF = new SimpleAttributeDefinitionBuilder(NestSubsystemExtension.NEST_NAME_ELEMENT,
	...
	private NestSubsystemDefinition() {
        super(NestSubsystemExtension.SUBSYSTEM_PATH, NestSubsystemExtension.getResourceDescriptionResolver(null), NestSubsystemAdd.INSTANCE,
    ...
    public void registerOperations(ManagementResourceRegistration rr) {
        ...
        rr.registerOperationHandler(
                new SimpleOperationDefinitionBuilder(NestSubsystemExtension.NEST_START_OP, NestSubsystemExtension.getResourceDescriptionResolver(null))
                        .addParameter(START_OP_PARAM_RESTART).build(), NestSubsystemStart.INSTANCE);

        rr.registerOperationHandler(
                new SimpleOperationDefinitionBuilder(NestSubsystemExtension.NEST_STOP_OP, NestSubsystemExtension.getResourceDescriptionResolver(null))
                        .build(), NestSubsystemStop.INSTANCE);

        rr.registerOperationHandler(
                new SimpleOperationDefinitionBuilder(NestSubsystemExtension.NEST_STATUS_OP, NestSubsystemExtension.getResourceDescriptionResolver(null))
